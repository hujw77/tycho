...
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Skipping clean_transaction_table cron registration because pg_cron is not installed in %',
            current_database();
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Skipping clean_transaction_table cron registration in database %: %',
            current_database(),
            SQLERRM;
END;
$$;
